"""Copyright
This code is based on https://github.com/D-X-Y/AutoDL-Projects/blob/master/lib/utils/flop_benchmark.py which is licensed under the MIT license.
"""
...
def main():
    ...
    import genotypes
    import utils
    from module.model import NetworkCIFAR, NetworkImageNet

    CIFAR_CLASSES = 10

    # CLI params
    parser = argparse.ArgumentParser("valid_adv_cifar")
    parser.add_argument('--genotype', type=str, required=True, help='name of genotype')
    parser.add_argument('--dataset', type=str, default='CIFAR', help='name of genotype')
    ...
    if args.dataset.lower() == 'cifar':
        ...
        model = NetworkImageNet(args.init_channels, CIFAR_CLASSES, args.layers, args.auxiliary, genotype)
    else:
        raise ValueError('wrong dataset type: %s' % args.dataset)
    model.drop_path_prob = 0.

    print("model param size = %fMB" % utils.count_parameters_in_MB(model))

    model_flops = get_model_infos(model, input_size)[0]

    print('model_flops = %fM' % model_flops)
