...
def train(dataloader, model, criterion, optimizer, scheduler, loss_scaler, epoch, local_rank, start_step, best_acc,
          save_path):
    model.train()
...
def main(local_rank):
    ...
    gpus = torch.cuda.device_count()  # 每台机器的GPU个数 the number of GPU on each GPU
    ...
    train_dataloader = train_dataset.loader(batch_size, sampler=train_datasampler, num_workers=num_workers,
                                            pin_memory=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train(train_dataloader, model, criterion, optimizer, scheduler, loss_scaler, epoch, local_rank, start_step,
              best_acc, save_path)
        start_step = 0
        scheduler.step()
